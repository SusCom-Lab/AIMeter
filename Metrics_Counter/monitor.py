...
def print_formatted_metrics(metrics: dict[str, any], task_name: str):
    ...
    # 写成英文
    print(f"📊 {'Task: ' + repr(task_name) + ' Performance Metrics Summary':^{SEPARATOR_LEN-16}} 📊") # 居中标题
    print("=" * SEPARATOR_LEN)
    ...
    # total_time_str = f"{total_time_float:.2f} 秒" if total_time_float is not None else "N/A"
    # print(f"  {'任务总耗时':<{LABEL_WIDTH}}: {total_time}")
    # 写成英文
    print(f"  {'Total Time':<{LABEL_WIDTH}}: {total_time}")
    ...
    # # 写成英文
    print(f"  {'CPU Energy':<{LABEL_WIDTH}}: {_format_value(energy_consumption.get('cpu_energy'), precision=3)} Joules")
    print(f"  {'DRAM Energy':<{LABEL_WIDTH}}: {_format_value(energy_consumption.get('dram_energy'), precision=3)} Joules")
    ...
    if gpu_energy:
        ...
        for gpu_idx in sorted(gpu_energy.keys(), key=lambda x: int(x) if str(x).isdigit() else float('inf')):
            ...
            # 写成英文
            print(f"  {f'GPU {gpu_idx} Energy':<{LABEL_WIDTH}}: {_format_value(energy, precision=3)} Joules")

    # print(f"  {'总能耗':<{LABEL_WIDTH-1}}: {_format_value(energy_consumption.get('total_energy'), precision=3)} Joules")
    # 写成英文
    print(f"  {'Total Energy':<{LABEL_WIDTH-1}}: {_format_value(energy_consumption.get('total_energy'), precision=3)} Joules")
    ...
    if cpu_dram_stats:
        # 指定CPU/DRAM指标的显示顺序
        cpu_dram_order = ['cpu_usage [%]', 'cpu_power [W]', 'dram_usage [%]', 'dram_power [W]'] # 示例名称，根据需要调整
...
def start(task_name: str, sampling_interval: float = 1, output_format: str = "csv", additional_metrics: list = [], indices: list = []):
    """
    启动监控：开始采集数据
